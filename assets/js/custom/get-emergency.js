( function( $ ) {
	$( document ).ready( function() {
		...
		const siteEnums = Object.freeze({
			...
			UW_FDL: 'https://uwosh.edu/fdl',
			UW_FOX: 'https://uwosh.edu/fox'
		});
				...
				success: function( response ) {
					...
					if ( ! $.isEmptyObject( broadcasts ) ) {
						...
						broadcasts.forEach( ( broadcast, index ) => {
							...
							broadcastDescription = $( broadcast.excerpt.rendered ).text();
		...
		if ( site === siteEnums.UW_FOX ) {
			...
			fetchEmergencies( `${categoryEnums.UW_FOX},${categoryEnums.UWO_CAMPUSES}` );
		} else if ( site === siteEnums.UW_FDL ) {

			// pull back emergencies for UW Fond Du Lac
			fetchEmergencies( `${categoryEnums.UW_FDL},${categoryEnums.UWO_CAMPUSES}` );
		} else if ( site === siteEnums.UW_OSHKOSH ) {

			// pull back emergencies for every category because we are on the home page
			fetchEmergencies( `${categoryEnums.UW_FOX},${categoryEnums.UW_FDL},${categoryEnums.UW_OSHKOSH},${categoryEnums.UWO_CAMPUSES}` );
		} else {
